...
void hybridSort( vector<Comparable> & a )
{
	...
	if(a.size() < 16){
		...
		int size1 = a.size()/4 - 1;
		int size2 = a.size()/4;
		int size3 = a.size()/4*2-1;
		int size4 = a.size()/4*2;
		int size5 = a.size()/4*3-1;
		int size6 = a.size()/4*3;
		int size7 = a.size()-1;
		quicksort(a, 0, size1);
		quicksort(a, size2, size3);
		quicksort(a, size4, size5);
		quicksort(a, size6, size7);
		
		vector<Comparable> tmpArray(a.size());
		merge(a, tmpArray, 0,size2, size3);
		merge(a, tmpArray, size4, size6, size7);
		merge(a, tmpArray, 0, size4, size7);
	}
	/*else if(a.size()%4==1){
		quicksort(a, 0, a.size()/4-1);
		quicksort(a, a.size()/4, a.size()/4*2-1);
		quicksort(a, a.size()/4*2, a.size()/4*3-1);
		quicksort(a, a.size()/4*3, a.size()-1);
		
		vector<Comparable> tmpArray(a.size());
		merge(a, tmpArray, 0, a.size()/4, a.size()/4*2-1);
		merge(a, tmpArray, a.size()/4*2, a.size()/4*3, a.size()-1);
		merge(a, tmpArray, 0, a.size()/4*2, a.size()-1);
	}
	else if(a.size()%4==2){
		quicksort(a, 0,a.size()/4-1);
		quicksort(a, a.size()/4, a.size()/4*2-1);
		quicksort(a, a.size()/4*2, a.size()/4*3);
		quicksort(a, a.size()/4*3+1, a.size()-1);
		
		vector<Comparable> tmpArray(a.size());
		merge(a, tmpArray, 0, a.size()/4, a.size()/4*2-1);
		merge(a, tmpArray, a.size()/4*2, a.size()/4*3+1, a.size()-1);
		merge(a, tmpArray, 0, a.size()/4*2, a.size()-1);
	}
	else if(a.size()%4==3){
		quicksort(a, 0, a.size()/4-1);
		quicksort(a, a.size()/4+1, a.size()/4*2);
		quicksort(a, a.size()/4*2+1, a.size()/4*3+1);
		quicksort(a, a.size()/4*3+2, a.size()-1);
		
		vector<Comparable> tmpArray(a.size());
		merge(a, tmpArray, 0,a.size()/4+1, a.size()/4*2);
		merge(a, tmpArray, a.size()/4*2+1, a.size()/4*3+2, a.size()-1);
		merge(a, tmpArray, 0, a.size()/4*2+1, a.size()-1);
	}*/
}
